...
from utils import bandpass_data_space, bandpass_data, plot_data, find_noise_idx, impute_noisy_trace
from data_classes import SurfaceWaveSelector
from imaging_classes import DispersionImagesFromWindows, VirtualShotGathersFromWindows

sys.path.append("/sep/syyuan/projects/car_tracking")
from tracking import KF_tracking
...
class TimeLapseImaging:
    ...
    def _preprocessing_for_surface_waves(self, impute_noise_traces=True, noise_threshold=5, impute_empty_traces=True):
        ...
        if impute_empty_traces:
            noisy_index = find_noise_idx(self.data_for_imaging, noise_threshold=10, empty_tr=True)
            impute_noisy_trace(self.data_for_imaging, noisy_index)

        if impute_noise_traces:
            noisy_index = np.argmax(np.max(self.data_for_imaging, axis=1) > noise_threshold)
            if noisy_index + 1 == self.data.shape[0]:
                self.data_for_imaging[noisy_index] = self.data_for_imaging[noisy_index - 1]
            elif noisy_index == 0:
                self.data_for_imaging[noisy_index] = self.data_for_imaging[noisy_index + 1]
            else:
                self.data_for_imaging[noisy_index] = (self.data_for_imaging[noisy_index - 1] + self.data_for_imaging[noisy_index + 1]) / 2
    ...
    def detect_cars(self, start_x, end_x, tracking_args, show_plot=True, reverse_amp=True, plt_xlim=1000, sigma_a=0.01):
        ...
        veh_base = self.tracking.detect_in_one_section(start_x=self.start_x, nx=15, sigma=0.08, show_plot=show_plot, plt_xlim=plt_xlim)
