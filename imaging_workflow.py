...
class ImagingWorkflowOneDirectory:
    ...
    def __init__(self, directory, tracking_args=None, method='surface_wave', imaging_IO_dict={}):
        self.directory = directory
        self.imagingIO = ImagingIO(directory, **imaging_IO_dict)
    ...
    def imaging(self, start_x, end_x, x0, wlen_sw=8, length_sw=300, spatial_ratio=0.75,
        ...
        for k, (data, x_axis, t_axis) in enumerate(self.imagingIO):
            ...
            imagingObj.detect_cars(start_x=start_x, end_x=end_x, tracking_args=tracking_args, show_plot=False)
            if verbal:
    ...
    def plot_intermediate_images(self, figsize=(8, 8), fig_dir='Fig/dispersion', x_lim=[-150, 150]):
        ...
        for k, result in enumerate(self.avg_images_to_save):
            ...
            ax.set_title(f"Time: {time_}m Number of Vehicles {n_cars}")
            name = f"time_{time_}m_nCars_{n_cars}"
            fname = f"vs_{name}.png"
            avg_image = result["avg_image"]
            avg_image.plot_image(fname, fig_dir=fig_folder, norm=True, ax=ax, x_lim=x_lim)
