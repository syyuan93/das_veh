...
class ImagingIO:
    ...
    # def __init__(self, directory, root='/net/brick6/data5/syyuan/time_lapse_windows/', ch1=400, ch2=540):
    def __init__(self, directory, root='/net/vision/scr3/syyuan/time_lapse_windows/', ch1=400, ch2=540, smoothing=True):
    ...
    def __getitem__(self, idx):
        ...
        data, x_axis, t_axis = utils._read_das_npz(file_path, ch1=self.ch1, ch2=self.ch2)
        scale = 1
        date = file_path.split('/')[-2]
        if date > '20230219':
            scale = 6463.81735715902
        if self.smoothing:
            data = savgol_filter(data, 20, 15)
        data /= scale
